...
class CharGrid:
    def __init__(self, img_src: ndarray):
        ...
        self.img_show, self.rect_loc = self.location(img_src)

        # 绘制水平线
        for row in range(self.rows):
            for col in range(self.cols):
                rect = self.get_rect(row, col)
                cv_utils.draw_rect(rect, self.img_show)
...
if __name__ == '__main__':
    ...
    cv2.imshow('located', char_grid.img_show)
    cv2.imshow('(0,1),(2,5)', char_grid.get_image(0, 1, 2, 5,(4,15)))
    cv2.waitKey()
