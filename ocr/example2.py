...
# 读取图像
image = Image.open('full4.bmp')

# 使用Pytesseract进行字符识别
text = pytesseract.image_to_string(image)
print(text)
# 将图像转换为OpenCV格式
image_cv = cv2.cvtColor(cv2.imread('full4.bmp'), cv2.COLOR_BGR2RGB)
...
for box in detection_boxes.splitlines():
    ...
    cv2.putText(image_cv, box[0], (x1, img_h - y1), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 255, 255), 2)
